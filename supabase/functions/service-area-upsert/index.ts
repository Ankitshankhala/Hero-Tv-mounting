...
serve(async (req) => {
  ...
  try {
    const { workerId, areaName, mode, polygon, zipcodesOnly, areaIdToUpdate } = await req.json() as UpsertRequest;
    ...
    // Handle ZIP-only mode
    if (zipcodesOnly && zipcodesOnly.length > 0) {
      console.log(`Processing ZIP-only mode with ${zipcodesOnly.length} ZIP codes`);
      zipcodes = zipcodesOnly;
    } else if (polygon && polygon.length >= 3) {
      console.log(`Processing polygon mode with ${polygon.length} points`);
      // Find ZIP codes within the polygon
      zipcodes = await findZipcodesWithCentroidFallback(polygon, supabase);
    } else {
      throw new Error('Either zipcodesOnly or polygon with at least 3 points is required');
    }

    if (zipcodes.length === 0) {
      throw new Error('No ZIP codes found for the given area');
    }

    // Handle existing area update
    if (areaIdToUpdate) {
      ...
      if (areaError || !existingArea) {
        throw new Error('Service area not found or not accessible');
      }

      serviceArea = existingArea;

      // Update area name if provided
      if (areaName && areaName !== existingArea.area_name) {
        const { error: updateError } = await supabase
          .from('worker_service_areas')
        ...
        if (updateError) {
          throw new Error(`Failed to update area name: ${updateError.message}`);
        }
      }
    } else {
    ...
    // Synchronize ZIP codes
    const syncResult = await synchronizeZipcodes(
      supabase,
      workerId,
      zipcodes,
      serviceArea.id,
      zipcodesOnly ? 'manual' : 'polygon',
      mode
    );

    // Clean up orphaned ZIP codes if in replace_all mode
    if (mode === 'replace_all') {
      await cleanupOrphanedZipcodes(supabase, workerId);
    }
    ...
    return new Response(
      JSON.stringify({
        success: false,
        error: error.message || 'Failed to process service area request'
      }),
      {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        status: 400,
