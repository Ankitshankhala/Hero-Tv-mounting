...
serve(async (req) => {
  ...
  try {
    ...
    while (hasMore) {
      ...
      console.log(`[STRIPE-SYNC] Fetching charges from Stripe API`);
      ...
      if (!resp.ok) {
        ...
        console.error("[STRIPE-SYNC] Stripe API error:", err);
      ...
      console.log(`[STRIPE-SYNC] Fetched ${data.data?.length || 0} charges`);
      ...
      for (const charge of data.data ?? []) {
        ...
        if (existErr) {
          console.error("[STRIPE-SYNC] Error checking existing transaction:", existErr);
        }
        if (existing) {
          console.log(`[STRIPE-SYNC] Transaction already exists for PI: ${payment_intent_id}`);
          continue;
        }
        ...
        if (bkErr) {
          console.error("[STRIPE-SYNC] Booking lookup error:", bkErr);
        }
        ...
        console.log(`[STRIPE-SYNC] Inserting transaction for PI: ${payment_intent_id}, amount: ${amount}`);
        
        const { error: insErr } = await supabase.from("transactions").insert(insertPayload);
        if (insErr) {
          console.error("[STRIPE-SYNC] Insert error:", insErr, insertPayload);
        } else {
    ...
    console.log(`[STRIPE-SYNC] Sync complete. Imported ${imported.length} transactions`);

    return new Response(
      JSON.stringify({ success: true, imported_count: imported.length, imported }),
