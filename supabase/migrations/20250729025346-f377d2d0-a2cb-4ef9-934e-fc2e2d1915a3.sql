-- Fix guest booking worker assignment by updating the function to handle guest_customer_info
-- Also add enhanced error recovery for the booking flow

-- Update the auto_assign_workers_with_coverage function to handle guest bookings properly
...
BEGIN
  ...
  -- Convert the scheduled date to proper day name for worker availability lookup
  -- Note: We need to handle the day_of_week enum properly
...
-- Add notification triggers for failed worker assignments
CREATE OR REPLACE FUNCTION public.notify_admin_of_assignment_failure()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
BEGIN
  -- Only trigger for bookings that remain pending after attempted assignment
  IF NEW.status = 'pending' AND OLD.status = 'payment_pending' THEN
    -- Log admin notification need
    INSERT INTO public.sms_logs (booking_id, recipient_number, message, status, error_message)
    VALUES (NEW.id, 'admin', 'Booking requires manual worker assignment - no automatic assignment possible', 'pending', NULL);
  END IF;
...
-- Create trigger for admin notifications on assignment failures (if it doesn't exist)
DROP TRIGGER IF EXISTS notify_admin_assignment_failure ON public.bookings;
CREATE TRIGGER notify_admin_assignment_failure
