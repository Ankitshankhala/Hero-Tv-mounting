-- Clean Database Migration - Phase 2 Revised (Complete Fix)
...
-- Step 3: Drop ALL triggers on bookings table
...
DROP TRIGGER IF EXISTS trigger_auto_invoice_on_booking_completion ON bookings;
DROP TRIGGER IF EXISTS validate_payment_authorization_trigger ON bookings;
DROP TRIGGER IF EXISTS auto_assign_worker_trigger ON bookings;
...
-- Step 8: Create validation function for booking/payment status consistency
...
-- Step 9: Create trigger for booking/payment status consistency
CREATE TRIGGER validate_booking_payment_consistency_trigger
  BEFORE INSERT OR UPDATE ON bookings
  FOR EACH ROW
  EXECUTE FUNCTION validate_booking_payment_consistency();

-- Step 10: Update payment authorization trigger for new workflow
...
-- Step 11: Recreate only essential triggers (updated for new workflow)
