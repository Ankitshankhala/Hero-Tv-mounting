...
-- Drop all the email notification related triggers in the correct order
DROP TRIGGER IF EXISTS bookings_email_notification_trigger ON public.bookings;
...
-- Create the new consolidated notification trigger function
...
-- Create the new trigger (replace the old one)
