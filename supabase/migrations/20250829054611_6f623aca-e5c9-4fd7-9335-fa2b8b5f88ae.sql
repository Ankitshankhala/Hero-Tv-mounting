...
-- 1. Update worker_service_areas to allow multiple active areas (remove single active constraint)
-- The existing table already supports multiple areas, we just need to update the assignment logic

-- 2. Add index for better performance on ZIP lookups
CREATE INDEX IF NOT EXISTS idx_worker_service_zipcodes_zipcode 
ON worker_service_zipcodes(zipcode);

CREATE INDEX IF NOT EXISTS idx_worker_service_zipcodes_worker_active 
ON worker_service_zipcodes(worker_id, zipcode) 
WHERE EXISTS (
...
-- 3. Create strict ZIP-based worker finder function
...
-- 4. Update auto-assignment function to use strict ZIP coverage
...
-- 5. Create helper function to get all active ZIP codes for a worker
...
-- 6. Create function to toggle service area active status
