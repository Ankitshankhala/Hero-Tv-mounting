-- Fix the CASE statement in validate_booking_payment_consistency function
  ...
  CASE NEW.status
    WHEN 'pending' THEN
      IF NEW.payment_status NOT IN ('pending') THEN
        RAISE EXCEPTION 'pending bookings must have pending payment_status, got: %', NEW.payment_status;
      END IF;
    ...
    WHEN 'in_progress' THEN
      -- Allow flexible payment status for in-progress bookings
      IF NEW.payment_status NOT IN ('authorized', 'completed', 'captured') THEN
