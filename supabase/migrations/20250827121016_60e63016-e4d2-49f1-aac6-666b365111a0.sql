...
CREATE UNIQUE INDEX CONCURRENTLY IF NOT EXISTS idx_email_logs_unique_booking_recipient 
ON email_logs (booking_id, recipient_email, email_type) 
WHERE status = 'sent';

-- Prevent duplicate active idempotency records
CREATE UNIQUE INDEX CONCURRENTLY IF NOT EXISTS idx_idempotency_unique_active 
ON idempotency_records (idempotency_key, operation_type, user_id) 
WHERE status IN ('pending', 'completed') AND expires_at > now();
