-- Replace trigger_booking_notifications with schema-aligned implementation
-- Fixes: Uses 'users' table instead of 'profiles', gets guest info from JSON, uses correct column names
-- Removes SMS logging to avoid enum issues and keep flow unblocked
...
BEGIN
  ...
  IF NEW.status IN ('confirmed', 'payment_authorized') 
    ...
    INSERT INTO public.email_logs (recipient_email, email_type, subject, body, status)
    VALUES (
      v_customer_email,
