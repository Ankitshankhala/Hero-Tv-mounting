-- Fix security issues: Add missing RLS policies and search paths
...
-- Step 2: Add basic read-only policies for spatial tables
CREATE POLICY "Anyone can read geometry_columns" ON public.geometry_columns
FOR SELECT USING (true);

CREATE POLICY "Anyone can read geography_columns" ON public.geography_columns
FOR SELECT USING (true);

CREATE POLICY "Anyone can read spatial_ref_sys" ON public.spatial_ref_sys
FOR SELECT USING (true);

-- Step 3: Fix search paths for new functions (replace existing ones)
