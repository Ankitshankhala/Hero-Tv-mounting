-- Fix revenue calculations to use 'completed' status (which is how captured payments are stored)
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    -- Total revenue from completed transactions (captured payments)
    'totalRevenue', COALESCE((
      SELECT SUM(amount) 
      FROM transactions 
      WHERE status IN ('completed', 'captured')
    ), 0),
    ...
    -- Revenue this month (captured/completed payments)
    ...
    -- Revenue last month (captured/completed payments)
