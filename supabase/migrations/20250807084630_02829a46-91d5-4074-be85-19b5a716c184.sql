-- Create a function to safely delete a booking with all related records
...
DECLARE
  ...
  manual_charges_count integer := 0;
  onsite_charges_count integer := 0;
...
BEGIN
  ...
  -- 1. Delete SMS logs (this was causing the constraint violation)
  ...
  -- 4. Delete manual charges
  DELETE FROM manual_charges WHERE booking_id = p_booking_id;
  GET DIAGNOSTICS manual_charges_count = ROW_COUNT;
  
  -- 5. Delete onsite charges
  DELETE FROM onsite_charges WHERE booking_id = p_booking_id;
  GET DIAGNOSTICS onsite_charges_count = ROW_COUNT;
  
  -- 6. Delete booking service modifications
  ...
  -- 7. Delete invoice service modifications
  ...
  result := jsonb_set(result, '{deleted_counts, manual_charges}', to_jsonb(manual_charges_count));
  result := jsonb_set(result, '{deleted_counts, onsite_charges}', to_jsonb(onsite_charges_count));
  ...
  result := jsonb_set(result, '{deleted_counts, booking}', to_jsonb(CASE WHEN booking_deleted THEN 1 ELSE 0 END));
