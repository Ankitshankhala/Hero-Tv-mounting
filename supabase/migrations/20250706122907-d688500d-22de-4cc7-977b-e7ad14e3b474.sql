-- Configure service role key setting for auto-invoice triggers
-- Note: In production, this should be set to the actual service role key
-- For now, we'll use a placeholder that the trigger can handle
ALTER SYSTEM SET app.settings.service_role_key = 'placeholder-will-use-anon-key';
SELECT pg_reload_conf();

-- Update the trigger function to use the anon key instead for now
