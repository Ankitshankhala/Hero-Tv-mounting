...
export const StripeCardElement = forwardRef<StripeCardElementRef, StripeCardElementProps>(
  ({ onReady, onError }, ref) => {
      ...
      return (
        <div className="p-4 border border-destructive/50 rounded-md bg-destructive/10">
          <div className="flex items-center space-x-2">
            <div className="text-destructive text-sm font-medium">Payment Setup Error</div>
          </div>
          <div className="text-destructive text-sm mt-1">{configError}</div>
          <button 
            onClick={handleRetry}
            className="mt-2 text-sm text-destructive hover:text-destructive/80 underline focus:outline-none"
          >
            Retry Setup
          </button>
        </div>
    ...
    if (isLoading) {
      return (
        <div className="p-4 border rounded-md bg-muted min-h-[52px] flex items-center justify-center">
    ...
    return (
      <div className="space-y-2">
        <div 
          ref={cardElementRef} 
          className="p-4 border border-input rounded-md bg-background min-h-[52px] focus-within:border-ring focus-within:ring-1 focus-within:ring-ring"
