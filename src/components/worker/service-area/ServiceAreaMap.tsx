...
const ServiceAreaMap = ({
  ...
  const [zipcodeTestResult, setZipcodeTestResult] = useState<{
    found: boolean;
    message: string;
  } | null>(null);
  ...
  const {
    ...
    fetchServiceAreas
  } = useWorkerServiceAreas(workerId);
  ...
  const loadServiceAreas = async () => {
    ...
    setLoading(true);
    try {
      const data = await optimizedSupabaseCall(`service-areas-${workerId}`, async () => {
        const {
          data,
          error
        } = await supabase.from('worker_service_areas').select('id, area_name, polygon_coordinates, is_active, created_at').eq('worker_id', workerId).order('created_at', {
          ascending: false
        });
      ...
      if (data && data.length > 0 && mapRef.current && drawnItemsRef.current) {
        ...
        activeAreas.forEach(area => {
          const coords = area.polygon_coordinates as Array<{
            lat: number;
            lng: number;
          }> | null;
          if (Array.isArray(coords) && coords.length >= 3) {
            ...
            if (areaZipCodes.length > 0) {
              ...
              const zipMarker = L.circleMarker([centerLat, centerLng], {
                color: '#10b981',
                // Green color for ZIP-only areas
                fillColor: '#10b981',
                fillOpacity: 0.6,
                radius: 6,
                weight: 2
  ...
  const displayPolygonOnMap = (coordinates: Array<{
    lat: number;
    lng: number;
  }>, area: ServiceArea) => {
  ...
  const handleSaveWithSelection = async () => {
    ...
    try {
      ...
      const {
        data,
        error
      } = await supabase.functions.invoke('service-area-upsert', {
        body: requestBody
      });
      if (error) throw error;
      if (!data.success) {
  ...
  const deleteServiceArea = async (areaId: string) => {
    try {
      const {
        error
      } = await supabase.from('worker_service_areas').delete().eq('id', areaId);
      if (error) throw error;
      toast({
        title: "Success",
  ...
  const searchForLocation = async () => {
    ...
    setSearching(true);
    try {
      const response = await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(searchAddress)}&limit=1&addressdetails=1&countrycodes=us`, {
        headers: {
  ...
  const testZipcodeInServiceArea = async () => {
    ...
    try {
      ...
      setZipcodeTestResult({
        found,
        message: found ? `✅ ZIP code ${testZipcode} is covered by your service area` : `❌ ZIP code ${testZipcode} is not in your service area`
      });
    } catch (error) {
      setZipcodeTestResult({
        found: false,
  ...
  const suggestZipFromMapCenter = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${center.lat}&lon=${center.lng}&addressdetails=1&zoom=18`, {
        headers: {
          'User-Agent': 'ServiceAreaMapper/1.0'
        }
      });
    ...
    return <Card className="border-blue-200 bg-blue-50">
        <CardContent className="pt-4">
          <div className="space-y-4">
            ...
            {/* Existing Areas */}
            {serviceAreas.length > 0 && <div className="space-y-2">
                <Label className="text-sm font-medium">Add to existing area:</Label>
                {serviceAreas.map(area => <div key={area.id} className={`p-3 rounded-lg border cursor-pointer transition-colors ${selectedExistingArea?.id === area.id ? 'border-blue-500 bg-blue-100' : 'border-gray-200 hover:border-gray-300'}`} onClick={() => {
              setSelectedExistingArea(area);
              setAreaSelectionMode('existing');
            }}>
                    <div className="flex items-center gap-3">
                      <input type="radio" checked={selectedExistingArea?.id === area.id} onChange={() => {
                  setSelectedExistingArea(area);
                  setAreaSelectionMode('existing');
                }} />
                      <div>
            ...
            <div className="space-y-2">
              ...
              <div className={`p-3 rounded-lg border cursor-pointer transition-colors ${areaSelectionMode === 'new' ? 'border-blue-500 bg-blue-100' : 'border-gray-200 hover:border-gray-300'}`} onClick={() => {
              setAreaSelectionMode('new');
              setSelectedExistingArea(null);
            }}>
                <div className="flex items-center gap-3">
                  <input type="radio" checked={areaSelectionMode === 'new'} onChange={() => {
                  setAreaSelectionMode('new');
                  setSelectedExistingArea(null);
                }} />
                  <div className="flex-1">
                    <p className="font-medium">Create new area</p>
                    <Input placeholder="Enter new area name" value={areaName} onChange={e => setAreaName(e.target.value)} onClick={e => e.stopPropagation()} className="mt-2" />
                  </div>
                </div>
              </div>
            </div>
  ...
  return <>
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-2 mb-4">
              <div className="flex-1 flex gap-2">
                <Input placeholder="Search for a location (e.g., 'Dallas, TX' or '123 Main St')" value={searchAddress} onChange={e => setSearchAddress(e.target.value)} onKeyDown={e => e.key === 'Enter' && searchForLocation()} />
                <Button onClick={searchForLocation} disabled={searching || !searchAddress.trim()} size="sm">
                  {searching ? <div className="w-4 h-4 border-2 border-current border-t-transparent rounded-full animate-spin" /> : <Search className="w-4 h-4" />}
                </Button>
              </div>
              
              {showLocationButton && <Button size="sm" variant="outline" onClick={centerOnUserLocation} disabled={locationLoading}>
                  {locationLoading ? <div className="w-4 h-4 border-2 border-current border-t-transparent rounded-full animate-spin" /> : <Globe className="w-4 h-4" />}
                  My Location
                </Button>}
            </div>
            ...
            <div className="relative">
              <div ref={mapContainerRef} className="w-full h-96 rounded-lg border" style={{
                minHeight: '400px'
              }} />
             </div>
            ...
            {showZipFallback && <Card className="border-orange-200 bg-orange-50">
                <CardContent className="pt-4">
                  <div className="space-y-4">
                    <div className="flex items-center gap-2">
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center justify-between">
                        ...
                        <Button variant="outline" size="sm" onClick={suggestZipFromMapCenter} disabled={!mapRef.current} className="text-xs">
                          <MapPin className="h-3 w-3 mr-1" />
                          Suggest from map center
                        </Button>
                      </div>
                      <Textarea id="manualZips" value={manualZipcodes} onChange={e => setManualZipcodes(e.target.value)} placeholder="75201, 75202, 75203..." className="min-h-[100px]" />
                    </div>
                    <div className="flex gap-2">
                      <Button onClick={() => {
                    ...
                    }} disabled={!manualZipcodes.trim()} variant="outline" className="flex-1">
                        Preview Polygon
                      </Button>
                      <Button onClick={saveZipcodesOnly} disabled={saving || !manualZipcodes.trim()} className="flex-1">
                        {saving ? <>
                            <div className="h-4 w-4 mr-2 animate-spin rounded-full border-b-2 border-white" />
                            Saving...
                          </> : <>
                    ...
                    {drawnPolygon && <Button onClick={() => {
                    // Convert drawn polygon to the format expected by saveServiceArea
                    if (drawnPolygon) {
                      const coordinates = drawnPolygon.map(coord => ({
                        lat: coord[0],
                        lng: coord[1]
                      }));
                      setCurrentPolygon(coordinates);
      ...
      {serviceAreas.length > 0 && <Card>
          <CardHeader>
            <CardTitle>Existing Service Areas</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-3">
              {serviceAreas.map(area => <div key={area.id} className={`flex items-center justify-between p-3 rounded-lg border ${area.is_active ? 'bg-primary/5 border-primary' : 'bg-muted'}`}>
                  <div className="flex items-center gap-3">
                    <div className={`w-3 h-3 rounded-full ${area.is_active ? 'bg-primary' : 'bg-muted-foreground'}`} />
                      <div>
                        <p className="font-medium">{area.area_name}</p>
                        <p className="text-sm text-muted-foreground">
                        ...
                        {area.is_active && <div className="flex items-center gap-2 mt-1">
                            <MapPinCheck className="h-3 w-3 text-primary" />
                            <Badge variant="secondary" className="text-xs">
                              {getServiceAreaZipCount(area.id)} ZIP codes in this area
                            </Badge>
                            {(!area.polygon_coordinates || !Array.isArray(area.polygon_coordinates) || area.polygon_coordinates.length < 3) && <Badge variant="outline" className="text-xs">
                                ZIP codes only
                              </Badge>}
                          </div>}
                      </div>
                  ...
                  <div className="flex items-center gap-2">
                    {area.polygon_coordinates && Array.isArray(area.polygon_coordinates) && area.polygon_coordinates.length >= 3 ? <Button variant="outline" size="sm" onClick={() => displayPolygonOnMap(area.polygon_coordinates, area)} title="Edit polygon area">
                        <Edit3 className="h-4 w-4" />
                      </Button> : <Button variant="outline" size="sm" disabled title="Cannot edit ZIP-only areas" className="opacity-50 cursor-not-allowed">
                        <Edit3 className="h-4 w-4" />
                      </Button>}
                    <Button variant="outline" size="sm" onClick={() => deleteServiceArea(area.id)}>
