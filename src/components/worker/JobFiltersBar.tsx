...
export const JobFiltersBar = ({
  ...
  const getStatusColor = (status: string) => {
    switch (status.toLowerCase()) {
      case 'scheduled': return 'bg-status-confirmed text-white';
  ...
  return (
    <Card className="bg-worker-card border-worker-border mb-6">
      <CardContent className="p-4 space-y-4">
        ...
        <div className="flex flex-col lg:flex-row gap-4">
          ...
          <div className="flex-1 relative">
            ...
            <Input
              placeholder="Search jobs by customer, address, or service..."
              className="pl-10 bg-background text-foreground border-worker-border"
        ...
        {availableStatuses.length > 0 && (
          <div className="space-y-2">
            <Label className="text-sm font-medium text-worker-card-foreground flex items-center gap-2">
              ...
              {availableStatuses.map((status) => (
                <div key={status} className="flex items-center gap-2">
                  ...
                  <Label 
                    ...
                    className="text-sm text-worker-card-foreground cursor-pointer"
                  >
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pt-2 border-t border-worker-border">
          <div className="flex items-center gap-4">
            ...
            {hasActiveFilters && (
              <Badge variant="secondary" className="bg-primary text-primary-foreground">
                {filters.statuses.length + filters.serviceTypes.length + 
                 (filters.afterLastJob ? 1 : 0) + 
                 (filters.searchTerm ? 1 : 0)} filters active
              </Badge>
            )}
          </div>
          ...
          {hasActiveFilters && (
            <Button
              ...
              className="text-worker-muted hover:text-worker-card-foreground"
            >
