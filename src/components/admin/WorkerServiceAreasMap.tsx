...
export const WorkerServiceAreasMap: React.FC<WorkerServiceAreasMapProps> = ({
  ...
  const generateConcaveHull = (zipCoordinates: ZipCoordinate[]) => {
    ...
    const points = zipCoordinates.filter(zip => zip.latitude && zip.longitude).map(zip => [zip.longitude, zip.latitude]);
    if (points.length < 3) return null;
    try {
  ...
  const fetchZipCoordinatesAndRender = async (zipCodes: string[], workerColor: string, area: Worker['service_areas'][0], worker: Worker, currentBounds: L.LatLngBounds | null) => {
    ...
    try {
      ...
      const {
        data: zipData,
        error
      } = await supabase.from('us_zip_codes').select('zipcode, latitude, longitude, city').in('zipcode', zipCodes);
      if (error) {
        console.warn('Error fetching ZIP coordinates:', error);
      }
      const foundZips = zipData || [];
      const missingZips = zipCodes.filter(zip => !foundZips.some(found => found.zipcode === zip));
      ...
      if (missingZips.length > 0) {
        ...
        for (let i = 0; i < missingZips.length; i += 3) {
          ...
          const batchResults = await Promise.all(batch.map(zip => geocodeZipcode(zip)));
          geocodedZips.push(...(batchResults.filter(Boolean) as ZipCoordinate[]));
        }
      }

      // Combine database and geocoded results
      const allCoordinates = [...foundZips.filter(z => z.latitude && z.longitude), ...geocodedZips];
  ...
  const syncPolygonToZips = async (area: Worker['service_areas'][0], worker: Worker) => {
    const areaZipCodes = worker.service_zipcodes.filter(zip => zip.service_area_id === area.id).map(zip => zip.zipcode);
    ...
    try {
      const {
        data: zipData,
        error
      } = await supabase.from('us_zip_codes').select('zipcode, latitude, longitude').in('zipcode', areaZipCodes);
      const foundZips = zipData || [];
      const missingZips = areaZipCodes.filter(zip => !foundZips.some(found => found.zipcode === zip));
      ...
      if (missingZips.length > 0) {
        for (let i = 0; i < missingZips.length; i += 3) {
          const batch = missingZips.slice(i, i + 3);
          const batchResults = await Promise.all(batch.map(async zip => {
      ...
      const allZipData = [...foundZips.filter(z => z.latitude && z.longitude), ...geocodedZips];
      ...
      if (hull.length >= 3) {
        ...
        const {
          error: updateError
        } = await supabase.from('worker_service_areas').update({
  ...
  const calculateConvexHull = (points: {
    ...
    for (let i = 1; i < points.length; i++) {
      if (points[i].lat < points[start].lat || points[i].lat === points[start].lat && points[i].lng < points[start].lng) {
        start = i;
      }
    }
    const hull: {
  ...
  return <>
      ...
      {selectedAreaInfo && <Card className="absolute top-4 right-4 max-w-sm z-[1000] shadow-lg">
          <CardContent className="p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={handleCloseAreaInfo} className="text-muted-foreground hover:text-foreground">
                ×
              </button>
            </div>
            ...
            <div className="space-y-2 text-sm">
              ...
              <div>
                ...
                <div className="text-xs text-muted-foreground mt-1 max-h-32 overflow-y-auto">
                  {selectedAreaInfo.zipCodes.length > 0 ? selectedAreaInfo.zipCodes.join(', ') : 'No ZIP codes assigned to this area'}
                </div>
              </div>
              ...
              {selectedAreaInfo.zipCodes.length >= 3 && <div className="mt-3 pt-3 border-t">
                  <Button size="sm" variant="outline" onClick={() => syncPolygonToZips(selectedAreaInfo.area, selectedAreaInfo.worker)}>
      ...
      {/* Legend */}
      {workers.length > 1 && !selectedWorkerId && <Card className="absolute bottom-4 left-4 z-[1000] shadow-lg">
          <CardContent className="p-3">
            <h5 className="font-medium text-sm mb-2">Workers</h5>
            <div className="space-y-1 max-h-32 overflow-y-auto">
              {workers.slice(0, 10).map((worker, index) => <div key={worker.id} className="flex items-center gap-2 text-xs">
              ...
              {workers.length > 10 && <div className="text-xs text-muted-foreground">
